...
import SundaeConnectButton from "./global/SundaeConnectButton.jsx";
import ConnectedButton from "./global/ConnectedButton.jsx";
...
const TableSection = ({ title, showDelegationInstead }) => {
  ...
  const TableRow = ({
    ...
    return (
      <tr
        className={`${
          index % 2 != 0 ? "bg-tableColor1" : "bg-tableColor2"
        } md:hover:bg-primary/10 duration-200 delay-80`}
      >
        {/* pool */}
        <td className={` ${rowstyle}`}>
          <div className="flex pr-20 md:pr-0 items-center">
            ...
            <span className="relative h-[25px] w-[40px] ml-3 mr-2">
              <img
                className="rounded-full size-[25px] absolute left-0"
                src="/sundaeYF.png"
              ></img>
              <img
                className="rounded-full size-[25px] absolute right-0 z-[2]"
                src="/liquid.png"
              ></img>
            </span>
                ...
                {version > 1 ? (
                  <span className="max-h-[18px] flex items-center text-[10px] text-secondary bg-secondary/20 px-2.5 rounded-full">
                    <svg
                      ...
                      aria-hidden="true"
                      data-slot="icon"
                      className="mr-[2px] h-3 w-3 text-secondary"
                    >
                      <path
                        strokeLinecap="round"
        ...
        <td className={rowstyle}>
          <span
            className={`${
              showDelegationInstead
                ? delegationStyle(delegation)
                : "pr-6 text-gray-200 text-sm"
            }`}
          >
        ...
        {/* tvl */}
        <td className={rowstyle}>
          <span className="pr-5   text-gray-200 text-xs md:text-sm">{tvl}</span>
        </td>
  ...
  return (
    <section className="mb-7 md:mb-16 ">
      <p
        className={`${
          title ? "mb-5 text-xl md:text-2xl font-bold text-gray-200" : "hidden"
        }`}
      >
        {title}
      </p>
      ...
      <div className="flex flex-wrap gap-2 mb-2 md:mb-4">
        ...
        <form className=" flex">
          <div className="relative flex items-center h-8">
            <input
              ...
              className="text-gray-200 active:border-2 focus:border-primary focus:outline-none transition-all duration-300 rounded-md pl-8 w-60 border border-gray-200/20 hover:border-primary/40 bg-tableColor2 text-sm placeholder-gray-200/40 h-full"
            ></input>
            <svg
              width="16"
              height="16"
              ...
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="text-gray-200 z-10 absolute bottom-1/2 translate-y-1/2 h-4 w-4 left-2"
            >
              <path
        ...
        <button className="text-nowrap min-h-8 min-w-[100px] px-4 py-auto text-sm text-gray-200 flex items-center gap-2 bg-buttonGray rounded-lg">
          <svg
            ...
            aria-hidden="true"
            data-slot="icon"
            className="h-4 w-4 will-change-transform group-hover:scale-110 transition-transform duration-300"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
          ...
          <svg
            ...
            aria-hidden="true"
            data-slot="icon"
            className="h-4 w-4 will-change-transform transition-transform duration-300"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
      ...
      <div className="space-y-4">
            ...
            <thead className="bg-tableColor1 text-gray-200/60 text-left text-xs h-[2.6rem] border-b border-gray-200/10">
              <tr className="pl-4">
                {[
                  "Pool",
                  "HRA",
                  `${showDelegationInstead ? "Community Delegation" : "Price"}`,
                  "TVL",
                  "Manage",
                ].map((item, index) => (
                  <th className="pl-4" key={index}>
                    <h4
                      className={`flex w-fit gap-1 items-center ${
                        item == "Community Delegation" ? "text-gray-200" : ""
                      }`}
                    >
                      {item}
                      <div class="inline">
                        ...
                        <svg
                          ...
                          aria-hidden="true"
                          data-slot="icon"
                          class="-mt-[2px] h-2 w-2 opacity-100"
                        >
                          <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
        ...
        <div className="w-full flex ">
          <button
            ...
            className="mx-auto py-1 px-4  rounded-xl text-gray-200 bg-buttonGray"
          >
            Load More
          </button>
        </div>
